# app.py (Complete - Standard Transformers loading with Offloading + Flag)
...
try:
    ...
    model = AutoModelForCausalLM.from_pretrained(
        LLM_MODEL_NAME,
        quantization_config=quantization_config, # Use the BNB config
        # device_map="auto",                  # Still use auto mapping
        # offload_folder=offload_directory,   # Specify where to offload files
        # offload_state_dict=True,            # Enable state dict offloading to disk
        # llm_int8_enable_fp32_cpu_offload=True # <<< ADDED BACK based on error message
    )
    # --- END MODIFIED ---
    ...
    if not isinstance(actual_context_limit, int) or actual_context_limit <= 0: actual_context_limit = 8192 # Sensible default
    print(f"DEBUG: Using context limit: {actual_context_limit}")


except Exception as e:
    print(f"FATAL ERROR: Could not load model {LLM_MODEL_NAME}: {e}")
    traceback.print_exc()
...
try:
    ...
    embedder = None; print("Warning: Embedder failed to load. Semantic deduplication will be disabled.")
...
def extract_text_from_pdf(pdf_path, detect_math=True, ocr_enabled=False):
    ...
    if not os.path.exists(pdf_path): raise FileNotFoundError(f"PDF file not found at: {pdf_path}")
...
# --- MODIFIED: generate_summary uses context limit & handles refinement flags ---
def generate_summary(text_chunks, grade_level_category, grade_level_desc, duration_minutes, has_math=False,
    ...
    if attempts == max_attempts and current_word_count < min_words: print(f"Warning: Max lengthening reached.")
...
def summarize_pdf():
    ...
    except torch.cuda.OutOfMemoryError: err = "GPU OOM during processing. Try smaller doc/settings."; print(f"Error: {err}"); traceback.print_exc(); return jsonify({"error": err}), 500
